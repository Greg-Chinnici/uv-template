...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Init a UV project with a specific template"
    )
    parser.add_argument("name", help="folder/project name")
    parser.add_argument(
        "template", help="template keyword or git url", nargs="?", default=None
    )
    args = parser.parse_args()

    template_url = (
        KNOWN_TEMPLATES.get(args.template, args.template) if args.template else None
    )
    ...
    if template_url:
        ...
        subprocess.run(["uv", "init"], cwd=final_dir, check=True)
